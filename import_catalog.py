...
from apps.products.models import ProductCategories, Products  # noqa: E402
...
catalog = {
    ...
    "FARINHA PANKO": ["Farinha Panko 1kg", "Farinha Panko Oriental 1020g"],
    "FARINHA DE ROSCA": ["Farinha de Rosca 5kg", "Farinha de Rosca 25 Kg"],
}
...
def run_import():
    ...
    for cat_name, products in catalog.items():
        ...
        for prod_name in products:
            ...
            counter = 1
            while (
                Products.objects.filter(slug=slug).exists()
                and not Products.objects.filter(name=prod_name).exists()
            ):
