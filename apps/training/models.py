...
class RestrictedMaterials(models.Model):
    ...
    created_by_user = models.ForeignKey(
        "users.Users",
        models.DO_NOTHING,
        related_name="restricted_materials_created",
        blank=True,
        null=True,
    )
    updated_by_user = models.ForeignKey(
        "users.Users",
        models.DO_NOTHING,
        related_name="restricted_materials_updated",
        blank=True,
        null=True,
    )
...
class RestrictedFiles(models.Model):
    restricted_material = models.ForeignKey(
        RestrictedMaterials,
        models.CASCADE,
        related_name="files",
    ...
    file_upload = models.FileField(
        upload_to="training/files/", verbose_name="Arquivo (Upload)"
    )
...
class RestrictedVideos(models.Model):
    ...
    video_url = models.CharField(
        max_length=500, blank=True, null=True, verbose_name="URL Externa (opcional)"
    )
    video_file = models.FileField(
        upload_to="training/videos/",
        blank=True,
        null=True,
        verbose_name="Arquivo de Vídeo (Upload)",
    )
    sort_order = models.IntegerField(default=0, verbose_name="Ordem de Exibição")
    created_at = models.DateTimeField(auto_now_add=True)
