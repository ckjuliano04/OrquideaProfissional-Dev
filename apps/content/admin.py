...
from .models import Brands, Pages, Stores, Tips
...
class BaseCMSAdmin(admin.ModelAdmin):
    ...
    def ações(self, obj):
        ...
        return mark_safe(
            f'<a href="{delete_url}" title="Excluir" style="color: #ba2121;">🗑️</a>'
        )
...
class StoresAdmin(BaseCMSAdmin):
    ...
    fieldsets = (
        (None, {"fields": ("title", "description", "is_active", "sort_order")}),
        (
            "Localização",
            {"fields": ("city", "state", "address_text", "latitude", "longitude")},
        ),
        ("Contato", {"fields": ("contact_info", "email")}),
    )
