...
class BaseCMSAdmin(admin.ModelAdmin):
    ...
    class Media:
        css = {"all": ("admin/css/cms_custom.css",)}
    ...
    def toggle_active(self, request, pk):
        ...
        if obj:
            ...
            obj.save()
            status = (
                "ativado" if obj.is_active else "desativado e movido para a lixeira"
            )
...
class ProductsAdmin(SortableAdminBase, BaseCMSAdmin):
    ...
    def image_preview_list(self, obj):
        if obj.image_url:
            return format_html(
                '<img src="{}" style="height: 40px; width: 40px; border-radius: 8px; object-fit: contain; background: #f8f9fa; border: 1px solid #dee2e6;"/>',
                obj.image_url.url,
            )
        return "-"
    ...
    def ver_no_site(self, obj):
        ...
        return format_html(
            '<a href="{}" target="_blank" style="background: #1a472a; color: white; padding: 5px 12px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 11px;">👁️ Ver Site</a>',
            url,
        )
    ...
    def image_preview(self, obj):
        if obj.image_url:
            return format_html(
                '<img src="{}" style="max-height: 200px;"/>', obj.image_url.url
            )
        return "-"
    ...
    fieldsets = (
        ...
        ("Mídia", {"fields": ("image_url", "image_preview")}),
    )
