...
class ProductCategories(models.Model):
    parent = models.ForeignKey(
        ...
        verbose_name="Divisão Mãe",
    )
    name = models.CharField(
        unique=True, max_length=100, db_collation="Latin1_General_CI_AS"
    )
...
class Products(models.Model):
    ...
    usage_tips = models.TextField(
        db_collation="Latin1_General_CI_AS",
        blank=True,
        null=True,
        verbose_name="Dicas de Uso",
    )
    ...
    shelf_life_info = models.CharField(
        max_length=100,
        db_collation="Latin1_General_CI_AS",
        blank=True,
        null=True,
...
class ProductImages(models.Model):
    ...
    image_url = models.ImageField(
        upload_to="uploads/products/", max_length=255, verbose_name="Imagem"
    )
...
class ProductFiles(models.Model):
    ...
    upload = models.ForeignKey(
        "core_media.Uploads", models.DO_NOTHING, blank=True, null=True
    )
    ...
    file_type = models.CharField(
        max_length=50, db_collation="Latin1_General_CI_AS", blank=True, null=True
    )
    sort_order = models.IntegerField()
    created_at = models.DateTimeField()
    external_url = models.CharField(
        max_length=500, db_collation="Latin1_General_CI_AS", blank=True, null=True
    )
...
class ProductRoleContents(models.Model):
    ...
    role = models.ForeignKey(
        "users.Roles", on_delete=models.CASCADE, db_column="role_id"
    )
...
class ProductNutrition(models.Model):
    ...
    serving_size = models.CharField(
        max_length=100, verbose_name="Porção (ex: 50g)", blank=True, null=True
    )
    household_measure = models.CharField(
        max_length=100,
        verbose_name="Medida Caseira (ex: 1/2 xícara)",
        blank=True,
        null=True,
    )
    portions_per_package = models.CharField(
        max_length=50, verbose_name="Porções por Embalagem", blank=True, null=True
    )
    ...
    column_count = models.IntegerField(
        choices=COLUMN_CHOICES, default=3, verbose_name="Número de Colunas no Site"
    )
    col_1_label = models.CharField(
        max_length=50, default="Valor por 100g", verbose_name="Título Coluna 1"
    )
    col_2_label = models.CharField(
        max_length=50, default="Valor por Porção", verbose_name="Título Coluna 2"
    )
    col_3_label = models.CharField(
        max_length=50, default="% VD", verbose_name="Título Coluna 3"
    )
    col_4_label = models.CharField(
        max_length=50,
        default="Extra 1",
        verbose_name="Título Coluna 4",
        blank=True,
        null=True,
    )
    col_5_label = models.CharField(
        max_length=50,
        default="Extra 2",
        verbose_name="Título Coluna 5",
        blank=True,
        null=True,
    )
...
class ProductNutritionRow(models.Model):
    ...
    label = models.CharField(
        max_length=100, verbose_name="Nutriente (ex: Carboidratos)"
    )
    value_100g = models.CharField(
        max_length=50, verbose_name="Valor Coluna 1", blank=True, null=True
    )
    value_serving = models.CharField(
        max_length=50, verbose_name="Valor Coluna 2", blank=True, null=True
    )
    vd_percentage = models.CharField(
        max_length=10, verbose_name="Valor Coluna 3", blank=True, null=True
    )
    value_4 = models.CharField(
        max_length=50, verbose_name="Valor Coluna 4", blank=True, null=True
    )
    value_5 = models.CharField(
        max_length=50, verbose_name="Valor Coluna 5", blank=True, null=True
    )
    ...
    if created:
        ...
        for order, label in default_rows:
            ProductNutritionRow.objects.create(
                product=instance.product, label=label, sort_order=order
            )
