...
export default function Home() {
  ...
  return (
    <div className="flex-grow flex flex-col">
      ...
      <section id="quem-somos" className="py-32 bg-orquidea-cream/30 scroll-mt-20 overflow-hidden">
          ...
          <div className="flex flex-col lg:flex-row gap-20 items-center">
            ...
            <div className="w-full lg:w-1/2">
              <ScrollReveal direction="right">
                <Badge variant="secondary" className="mb-6">Nossa História</Badge>
                <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-8 leading-tight font-serif tracking-tight">{data.about_title}</h2>
                ...
                <div className="grid grid-cols-2 gap-10 mt-12 pt-10 border-t border-slate-200">
                  <motion.div whileHover={{ y: -5 }}>
                    <h4 className="font-black text-orquidea-green text-4xl mb-1 tracking-tighter">{data.about_stat1_title}</h4>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{data.about_stat1_desc}</p>
                  </motion.div>
                  <motion.div whileHover={{ y: -5 }}>
                    <h4 className="font-black text-orquidea-green text-4xl mb-1 tracking-tighter">{data.about_stat2_title}</h4>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{data.about_stat2_desc}</p>
                  </motion.div>
                </div>
              </ScrollReveal>
            </div>
      ...
      <section id="dicas" className="py-32 bg-white scroll-mt-20">
        <div className="container mx-auto px-4 max-w-6xl text-center">
          <ScrollReveal>
            <Badge variant="outline" className="mb-6">Conhecimento Técnico</Badge>
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-20 font-serif tracking-tight">Dicas de Uso</h2>
          </ScrollReveal>
              ...
              <ScrollReveal key={tip.id} delay={index * 0.2}>
                ...
                >
                  ...
                  <div className="p-10 flex flex-col flex-grow">
                    <h3 className="font-black text-2xl text-slate-900 mb-4 group-hover:text-orquidea-green transition-colors leading-tight">{tip.title}</h3>
                    <p className="text-slate-500 mb-10 text-sm leading-relaxed flex-grow font-medium">{tip.description}</p>
                    <Link href="/dicas" className="group/link inline-flex items-center gap-3 text-orquidea-green font-black text-xs uppercase tracking-widest">
                      Ver Dica Completa 
                      <motion.span 
                        animate={{ x: [0, 5, 0] }}
                        transition={{ duration: 2, repeat: Infinity }}
          ...
          <ScrollReveal delay={0.5}>
            <div className="mt-20">
              <Link href="/dicas">
                <Button variant="outline" size="xl" className="rounded-2xl px-12 gap-3 border-slate-200 text-slate-600 hover:border-orquidea-green hover:text-orquidea-green">
      ...
      <section id="orquidea-alimentos" className="py-24 bg-orquidea-cream/20 scroll-mt-20 border-t border-slate-100">
        <div className="container mx-auto px-4 max-w-6xl text-center">
          <ScrollReveal>
            <h2 className="text-3xl md:text-4xl font-black text-slate-900 mb-6 font-serif">Família Orquídea Alimentos</h2>
          ...
          >
            {data.brands?.map((brand, index) => (
              ...
              >
                <a href={brand.link || '#'} target={brand.link ? "_blank" : "_self"} className="w-40 h-40 bg-white rounded-full shadow-md hover:shadow-2xl hover:border-orquidea-green border border-slate-100 flex flex-col items-center justify-center transition-all duration-500 group overflow-hidden">
                  <span className="font-black text-slate-800 group-hover:text-orquidea-green text-center px-4 tracking-tighter leading-tight transition-colors">{brand.name}</span>
                  {brand.link && (
      ...
      {/* ONDE COMPRAR */}
      <section id="onde-comprar" className="py-32 bg-orquidea-night text-white scroll-mt-16 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 max-w-4xl text-center relative z-10">
          <ScrollReveal>
            <h2 className="text-4xl md:text-5xl font-black mb-8 font-serif tracking-tight">{data.where_to_buy_title}</h2>
            ...
            <Link href="/onde-comprar">
              ...
              >
                ...
                <h4 className="text-[10px] font-black uppercase tracking-[0.4em] mb-8 text-orquidea-gold">Encontre um Parceiro Próximo</h4>
                <p className="text-4xl md:text-6xl font-black text-white mb-12 tracking-tighter transition-transform group-hover:scale-105 duration-500 leading-tight">
                  Consultar Listagem <br className="hidden md:block" /> de Vendedores
                </p>
