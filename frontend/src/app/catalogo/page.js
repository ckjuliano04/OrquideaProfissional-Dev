...
export default function CatalogoPage() {
  ...
  useEffect(() => {
    Promise.all([fetchAPI("/products/categories/"), fetchAPI("/products/")])
  ...
  const filteredProducts = products
    .filter((p) => {
      ...
      if (activeCategory.type === "division") {
        const childCatIds = subcategories
          .filter((s) => s.parent === activeCategory.id)
          .map((s) => s.id);
        return (
          (childCatIds.includes(p.category) ||
  ...
  const FilterContent = () => (
    <div className="flex flex-col gap-2">
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
        onClick={() => {
          setActiveCategory({ id: "all", type: "all", name: "Todos" });
          setDrawerOpen(false);
        }}
        className={cn(
      ...
      {mainDivisions.map((division) => (
        <div key={division.id} className="space-y-1">
          ...
          <div className="pl-4 flex flex-col gap-1 border-l border-slate-100 ml-4 mt-1">
            {subcategories
              .filter((sub) => sub.parent === division.id)
              .map((sub) => (
                <motion.button
                  key={sub.id}
                  whileHover={{ x: 3 }}
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
              ...
              >
                <nav className="flex items-center gap-2 text-[10px] font-black text-slate-400 mb-3 uppercase tracking-widest">
                  <span className="hover:text-orquidea-green cursor-pointer">
                    Catálogo
                  </span>
                  <ChevronRight size={12} className="text-slate-300" />
                  <span className="text-orquidea-green">
                    {activeCategory.name}
                  </span>
                </nav>
              ...
              {filteredProducts.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-slate-500 max-w-xs mx-auto">
                    Tente ajustar seus filtros ou mude o termo de busca.
                  </p>
                  <Button
                    variant="ghost"
                    className="mt-8"
                      ...
                      <Card className="group overflow-hidden flex flex-col border-none bg-white shadow-md hover:shadow-2xl h-full">
                        ...
                        <CardHeader className="flex-grow">
                          ...
                          <p className="text-sm text-slate-500 line-clamp-3 mb-4 leading-relaxed font-medium">
                            {product.short_description ||
                              "Produto desenvolvido com alta tecnologia para performance profissional superior."}
                          </p>
                        </CardHeader>
                        ...
                        <div className="px-8 pb-8">
                          {isAuthenticated ? (
                            ...
                            <Badge
                              variant="outline"
                              className="w-full justify-center py-3 border-dashed opacity-60"
                            >
