    ...
    <div className="min-h-screen bg-slate-50 flex flex-col pt-32">
      ...
      {isAdmin && (
        ...
        >
          ...
          >
            ...
            <span className="text-xs uppercase tracking-widest">Editar no CMS</span>
          </a>
        </motion.div>
      )}
      ...
      <main className="container mx-auto px-4 py-12 flex-grow">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="w-full lg:w-1/2 lg:sticky lg:top-32">
            ...
            <motion.div 
              ...
              className="mt-12"
            >
              <Button variant="destructive" size="xl" className="w-full rounded-2xl py-8 shadow-2xl shadow-orquidea-red/20 group">
                Onde Encontrar este Produto
                <ShoppingBag size={20} className="ml-3 group-hover:rotate-12 transition-transform" />
              </Button>
            </motion.div>
          </motion.div>
          ...
          <div className="w-full lg:w-1/2">
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-16">
              <Card className="border-none bg-white shadow-md hover:shadow-xl transition-all rounded-3xl p-8 flex items-center gap-6 group">
                ...
                <div>
                  <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Peso Líquido</span>
                  <span className="text-xl font-black text-slate-900">{product.weight_info || 'N/A'}</span>
                </div>
              </Card>
              <Card className="border-none bg-white shadow-md hover:shadow-xl transition-all rounded-3xl p-8 flex items-center gap-6 group">
                <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-orquidea-green group-hover:bg-orquidea-green group-hover:text-white transition-all">
                ...
                <div>
                  <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Validade</span>
                  <span className="text-xl font-black text-slate-900">{product.shelf_life_info || 'N/A'}</span>
                </div>
              </Card>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="space-y-8">
              <div className="flex items-center justify-between px-2">
                ...
                {product.nutrition && (
                  <Badge variant="outline" className="text-[10px] opacity-60">Padrão ANVISA</Badge>
                )}
              </div>

              <AnimatePresence mode="wait">
                {product.nutrition_rows && product.nutrition_rows.length > 0 ? (
                  <Card key="nutrition-table" className="border-none shadow-2xl overflow-hidden rounded-[2.5rem] bg-white">
                    <div className="p-10 border-b border-slate-100 flex flex-col sm:flex-row justify-between gap-6">
                      <div className="space-y-2">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Porções por embalagem</p>
                        <p className="text-xl font-black text-slate-900">{product.nutrition?.portions_per_package || 'Consultar'}</p>
                      </div>
                      <div className="space-y-2">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Medida Caseira</p>
                        <p className="text-xl font-black text-slate-900">{product.nutrition?.household_measure || 'Consultar'}</p>
                      </div>
                    </div>
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="bg-slate-50/50 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100">
                            ...
                            {product.nutrition?.column_count >= 1 && <th className="px-6 py-6 text-center">{product.nutrition.col_1_label}</th>}
                            {product.nutrition?.column_count >= 2 && <th className="px-6 py-6 text-center text-slate-900">{product.nutrition.col_2_label}</th>}
                            {product.nutrition?.column_count >= 3 && <th className="px-6 py-6 text-center text-orquidea-green">%VD*</th>}
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                          {product.nutrition_rows.map((row, idx) => (
                            ...
                            >
                              <td className="px-10 py-5 text-sm font-bold text-slate-700">{row.label}</td>
                              {product.nutrition?.column_count >= 1 && <td className="px-6 py-5 text-center text-sm text-slate-500 font-medium">{row.value_100g || '-'}</td>}
                              {product.nutrition?.column_count >= 2 && <td className="px-6 py-5 text-center text-sm text-slate-900 font-black">{row.value_serving || '-'}</td>}
                              {product.nutrition?.column_count >= 3 && <td className="px-6 py-5 text-center text-sm text-orquidea-green font-black">{row.vd_percentage || '-'}</td>}
                            </motion.tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <Card key="nutrition-empty" className="p-20 bg-white border-4 border-dashed border-slate-100 rounded-[3rem] flex flex-col items-center justify-center text-center">
                    <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center text-slate-300 mb-6">
                      <Info size={32} />
                    </div>
                    <p className="font-black text-slate-400 uppercase tracking-widest text-[10px]">Tabela nutricional em processo de atualização</p>
                  </Card>
                )}
              </AnimatePresence>
            </motion.div>
            ...
            {product.full_description && (
              <motion.div variants={itemVariants} className="mt-20 pt-16 border-t border-slate-200">
                <h3 className="text-2xl font-black text-slate-900 mb-8 font-serif">Informações Complementares</h3>
            ...
            {/* Support Action */}
            <motion.div variants={itemVariants} className="mt-20 p-10 bg-orquidea-night rounded-[2.5rem] text-white flex flex-col md:flex-row items-center gap-8 shadow-2xl relative overflow-hidden group">
              ...
              <div className="flex-grow text-center md:text-left relative z-10">
                <p className="text-orquidea-gold text-[10px] font-black uppercase tracking-[0.3em] mb-2">Dúvida Técnica?</p>
                <h4 className="text-2xl font-black mb-2 tracking-tight">Fale com um Especialista Orquídea</h4>
                <p className="text-white/50 text-sm">Nossos técnicos estão prontos para auxiliar na sua receita.</p>
              </div>
              <Button variant="glass" className="relative z-10 group-hover:scale-105 transition-transform">
                Entrar em Contato
