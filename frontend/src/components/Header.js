  ...
  const navLinks = [
    ...
    ...(isAuthenticated
      ? [
          ...
          ...(user?.role !== "CLIENTE"
            ? [{ href: "/treinamentos", label: "Treinos" }]
            : []),
        ]
      : []),
  ];
    ...
    >
      <div className="container mx-auto px-4 h-full flex items-center justify-between">
        ...
        <Link
          ...
          aria-label="Orquídea Profissional Home"
        >
          <Image
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-4 pl-8 border-l border-white/10">
              <div className="text-right hidden lg:block">
                <p className="text-[10px] font-black text-white/40 leading-tight uppercase tracking-widest mb-0.5">
                  {user?.role}
                </p>
              ...
              <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center text-white border border-white/10 shadow-inner group relative overflow-hidden transition-all hover:border-orquidea-gold/50">
                <User
                  size={20}
                  className="z-10 group-hover:text-orquidea-gold transition-colors"
                />
      ...
      >
        <nav
          className="container mx-auto px-6 py-12 flex flex-col gap-4"
            ...
            {isAuthenticated ? (
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-orquidea-gold font-bold uppercase">
                    {user?.role}
                  </p>
                </div>
                <Button
