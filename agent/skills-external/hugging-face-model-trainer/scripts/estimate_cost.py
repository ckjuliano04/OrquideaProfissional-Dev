...
def parse_args():
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Model name or size (e.g., 'Qwen/Qwen2.5-0.5B' or '0.5B')",
    )
    ...
    parser.add_argument(
        "--dataset-size", type=int, help="Override dataset size (number of examples)"
    )
    parser.add_argument(
...
def main():
    ...
    estimated_hours = estimate_training_time(
        model_params, dataset_size, args.epochs, args.hardware
    )
    ...
    if model_params >= 7 and args.hardware not in [
        "a10g-largex2",
        "a10g-largex4",
        "a100-large",
    ]:
