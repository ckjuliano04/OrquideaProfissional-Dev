...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Inspect dataset format for TRL training"
    )
...
def check_sft_compatibility(columns: List[str]) -> Dict[str, Any]:
    ...
    possible_prompt = find_columns(
        columns, ["prompt", "instruction", "question", "input"]
    )
    possible_response = find_columns(
        columns, ["response", "completion", "output", "answer"]
    )
...
def check_kto_compatibility(columns: List[str]) -> Dict[str, Any]:
    ...
    return {
        "ready": "prompt" in columns and "completion" in columns and "label" in columns
    }
...
def main():
    ...
    if sft_info["ready"]:
        ...
        print("  Action: Use directly with SFTTrainer")
    elif sft_info["possible_prompt"]:
        print(
            f"  Detected: prompt='{sft_info['possible_prompt']}' response='{sft_info['possible_response']}'"
        )
        print("  Action: Apply mapping code (see below)")
    else:
        print("  Status: Cannot determine mapping - manual inspection needed")
    ...
    if dpo_info["ready"]:
        print("  Reason: Dataset has 'prompt', 'chosen', 'rejected' fields")
        print("  Action: Use directly with DPOTrainer")
    elif dpo_info["can_map"]:
        print(
            f"  Detected: prompt='{dpo_info['prompt_col']}' chosen='{dpo_info['chosen_col']}' rejected='{dpo_info['rejected_col']}'"
        )
        print("  Action: Apply mapping code (see below)")
    else:
        print("  Status: Missing required fields (prompt + chosen + rejected)")
    ...
    if grpo_info["ready"]:
        print("  Reason: Dataset has 'prompt' field")
        print("  Action: Use directly with GRPOTrainer")
    elif grpo_info["can_map"]:
        print(f"  Detected: prompt='{grpo_info['prompt_col']}'")
        print("  Action: Apply mapping code (see below)")
    else:
        print("  Status: Missing prompt field")

    # KTO
    print(f"\n[KTO] {'✓ READY' if kto_info['ready'] else '✗ INCOMPATIBLE'}")
    if kto_info["ready"]:
        print("  Reason: Dataset has 'prompt', 'completion', 'label' fields")
        print("  Action: Use directly with KTOTrainer")
    else:
        print("  Status: Missing required fields (prompt + completion + label)")
    ...
    print(f"\n{'SUMMARY':-<80}")
    print(
        f"Recommended training methods: {', '.join(recommended) if recommended else 'None (dataset needs formatting)'}"
    )
