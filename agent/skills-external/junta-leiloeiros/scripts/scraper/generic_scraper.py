    ...
    async def parse_leiloeiros(self) -> List[Leiloeiro]:
            ...
            for row in rows[1:]:
                ...
                results.append(
                    self.make_leiloeiro(
                        nome=nome,
                        matricula=gcol(
                            cells, ["matr", "registro", "núm", "numero", "nº"]
                        ),
                        cpf_cnpj=gcol(cells, ["cpf", "cnpj", "documento"]),
                        situacao=gcol(cells, ["situ", "status"]),
                        municipio=gcol(cells, ["munic", "cidade"])
                        or self.municipio_default,
        ...
        if not results:
            list_items = soup.select(
                "ul.leiloeiros li, ol.leiloeiros li, .lista-leiloeiros li"
            )
            if not list_items:
            ...
            for li in list_items:
                ...
                if not text or len(text) < 5:
                    continue
                results.append(
                    self.make_leiloeiro(nome=text, municipio=self.municipio_default)
                )
