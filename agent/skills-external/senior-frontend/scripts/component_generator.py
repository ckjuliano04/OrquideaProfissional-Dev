...
def main():
    ...
    parser.add_argument(
        "--with-story", action="store_true", help="Generate Storybook story file"
    )
    parser.add_argument(
        "--no-index", action="store_true", help="Skip generating index.ts file"
    )
    parser.add_argument(
        "--flat",
        action="store_true",
    ...
    if args.dry_run:
        ...
        print(
            f"  Directory: {output_dir / pascal_name if not args.flat else output_dir}"
        )
