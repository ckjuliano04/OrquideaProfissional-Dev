...
_DOCKER_FROM_LATEST_RE = re.compile(r"""(?::latest\s*$|^[^:]+\s*$)""")
...
def analyze_requirements_txt(filepath: Path, verbose: bool = False) -> dict:
    ...
    for line_num, raw_line in enumerate(text.splitlines(), start=1):
        ...
        if (
            _PY_COMMENT_RE.match(line)
            or _PY_OPTION_RE.match(line)
            or _PY_BLANK_RE.match(line)
        ):
            continue
    ...
    if isinstance(scripts, dict):
        for script_name, script_cmd in scripts.items():
            ...
            if script_name in (
                ...
                "preinstall",
                "install",
            ) and _NODE_RISKY_SCRIPTS.search(script_cmd):
...
def analyze_dockerfile(filepath: Path, verbose: bool = False) -> dict:
    ...
    for line_num, raw_line in enumerate(lines, start=1):
        ...
        if from_match:
            ...
            is_trusted = any(
                image_core.startswith(prefix)
                or image_core.startswith(f"docker.io/library/{prefix}")
...
def discover_dependency_files(target: Path) -> list[Path]:
    ...
    for root, dirs, filenames in os.walk(target):
        ...
        for fname in filenames:
            ...
            if any(
                fname_lower.startswith(prefix.lower()) for prefix in DOCKER_PREFIXES
            ):
                found.append(fpath)
                continue
...
def format_text_report(
    ...
    displayed = [
        f
        for f in findings
        if config.SEVERITY.get(f.get("severity", "INFO"), 0)
...
def run_scan(
    ...
    if len(all_findings) > max_report:
        logger.warning(
            "Truncating findings from %d to %d", len(all_findings), max_report
        )
