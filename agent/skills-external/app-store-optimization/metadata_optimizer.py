...
class MetadataOptimizer:
    ...
    def optimize_title(
        ...
        if target_keywords:
            ...
            option2 = self._build_title_with_keywords(
                app_name, [primary_keyword], max_length
            )
            if option2:
                title_options.append(
                    {
        ...
        if len(target_keywords) > 1:
            option3 = self._build_title_with_keywords(
                app_name, target_keywords[:2], max_length
            )
            if option3:
                title_options.append(
                    {
    ...
    def optimize_keyword_field(
        ...
        return {
            ...
            "keywords_excluded": [
                kw for kw in target_keywords if kw.lower() not in keyword_field
            ],
    ...
    def calculate_keyword_density(
        ...
        total_keyword_occurrences = sum(
            kw["occurrences"] for kw in keyword_densities.values()
        )
        overall_density = (
            (total_keyword_occurrences / total_words * 100) if total_words > 0 else 0
        )
        ...
        return {
            ...
            "recommendations": self._generate_density_recommendations(
                keyword_densities
            ),
        }
    ...
    def _optimize_short_description(
        ...
        return {
            ...
            "keywords_included": [primary_keyword]
            if primary_keyword in short_desc.lower()
            else [],
    ...
    def _optimize_subtitle(
        ...
        primary_keyword = target_keywords[0] if target_keywords else ""
        key_feature = (
            app_info.get("key_features", [""])[0]
            if app_info.get("key_features")
            else ""
        )
    ...
    def _optimize_full_description(
        ...
        max_length = self.limits.get(
            "description", self.limits.get("full_description", 4000)
        )
        ...
        unique_value = app_info.get("unique_value", "")
        hook = (
            f"{unique_value} {primary_keyword.title()} that helps you achieve more.\n\n"
        )
...
def optimize_app_metadata(
    ...
    return {
        "platform": platform,
        "title": optimizer.optimize_title(app_info["name"], target_keywords),
        "description": optimizer.optimize_description(
            app_info, target_keywords, "full"
        ),
        "keyword_field": optimizer.optimize_keyword_field(target_keywords)
        if platform == "apple"
        else None,
    }
