...
class LocalizationHelper:
        ...
        for field, text in source_metadata.items():
            ...
            localized_metadata[field] = {
                ...
                "translation_notes": self._get_translation_notes(
                    field, target_language, estimated_length, limit
                ),
            }
    ...
    def calculate_localization_roi(
        ...
        payback_months = (
            (localization_cost / monthly_additional_revenue)
            if monthly_additional_revenue > 0
            else float("inf")
        )
        ...
        return {
            ...
            "annual_roi": f"{((monthly_additional_revenue * 12 - localization_cost) / localization_cost * 100):.1f}%"
            if payback_months != float("inf")
            else "Negative",
...
def plan_localization_strategy(
    ...
    target_markets = helper.identify_target_markets(
        current_market=current_market, budget_level=budget_level
    )
    ...
    return {"target_markets": target_markets, "roi_analysis": roi_analysis}
